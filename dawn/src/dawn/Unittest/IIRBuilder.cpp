...
IIRBuilder::build(std::string const& name, std::unique_ptr<iir::Stencil> stencil_) {
  DAWN_ASSERT(si_);
  // setup the whole stencil instantiation
  auto stencil_id = stencil_->getStencilID();
  si_->getMetaData().setStencilName(name);
  si_->getIIR()->insertChild(std::move(stencil_), si_->getIIR());
  ...
  for(const auto& stagePtr : iterateIIROver<iir::Stage>(*(si_->getIIR()))) {
    iir::Stage& stage_ = *stagePtr;
    for(const auto& doMethod : stage_.getChildren()) {
      doMethod->update(iir::NodeUpdateType::level);
    }
    stage_.update(iir::NodeUpdateType::level);
  }
  for(const auto& stagePtr : iterateIIROver<iir::Stage>(*(si_->getIIR()))) {
